...
const decency = (((progressSumLek + progressSumPrak) / (full_lection + full_prakt) * 100) + (rep / 5)) * (0.75 + (0.05 * multiplier)) + reason
...
| **Поведение** | **${rep}** баллов из **100**, +**${rep / 5}** к порядочности|
